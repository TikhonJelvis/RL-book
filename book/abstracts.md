...
# Programming and Design (unnumbered chapter)
...
# Chapter 9: Monte-Carlo and Temporal-Difference for Prediction

Chapter author: Ashwin Rao (https://orcid.org/0000-0003-0620-3100)

## Abstract

This chapter provides the entry into the world of Reinforcement Learning (RL) algorithms. The chapter starts by restricting the RL problem to a very simple one – one where the state space is small and manageable as a table enumeration (known as tabular RL) and where one has to calculate the Value Function for a Fixed Policy (known as the Prediction problem). The restriction to Tabular Prediction makes it very easy to understand the core concepts of Monte-Carlo (MC) and Temporal-Difference (TD). This is then extended to the problem of Prediction with function approximation, enabling handling of large state spaces. A comparison and contrast is made between the MC and TD techniques. The chapter ends with coverage of the TD(lambda) algorithm which enables one to find a continuum of algorithms between MC and TD by tuning the lambda parameter and playing the bias-variance tradeoff. 

# Chapter 10: Monte-Carlo and Temporal-Difference for Control
...
# Chapter 11: Batch RL, Experience-Replay, DQN, LSPI, Gradient TD
...
This chapter brings the various pieces of planning and learning concepts learned in this book together. The chapter starts by illustrating a methodology that works well in practice - creatively blending planning and learning (a technique known as Model-based RL). This is followed by coverage of the Monte Carlo Tree Search (MCTS) algorithm that was highly popularized when it solved the Game of GO, a problem that was thought to be insurmountable by current AI technology. The chapter ends with coverage of the Adaptive Multi-stage Sampling algorithm, that can be considered to be the "spiritual origin" of MCTS.
...
This appendix introduces moment generating functions (MGF) and derives some key results pertaining to MGFs that are useful in evaluating some expectations under a normal distribution. The appendix also derives some results on minimizing the MGF for a normal distribution and for a symmetric binary distribution.
...
# Appendix E: Black-Scholes Equation and it's Solution for Call/Put Options
