...
We know that maximizing $\mathbb{E}[U(W_1)]$ is equivalent to maximizing the $\log$ of the Certainty-Equivalent Value $W_$, which in this case (using the formula for $x_{CE}$ from the section on CRRA) is given by:
