...
The original Binomial Options Pricing Model was developed to price (and hedge) derivatives on an underlying whose price evolves according to a lognormal stochastic process, with the stochastic process approximated in the form of a simple discrete-state process that enables enormous computational tractability. The lognormal stochastic process is basically of the same form as the stochastic process of the underlying price in the Black-Scholes Model (covered in Appendix [-@sec:black-scholes-appendix]). However, the underlying price process in the Black-Scholes is specified in the real-world probability measure whereas here we specify the underlying price process in the risk-neutral probability measure. This is because here we will employ the method of discounted expectation (under the risk-neutral probability measure) of the derivative payoff. Recall that in the single-period setting, the underlying asset price grows (in expectation) at the riskless rate $r$. This applies even in the multi-period and continuous-time setting. For a continuous-time lognormal stochastic process, the lognormal drift will hence be $r$ in the risk-neutral probability measure (rather than $\mu$ in the real-world probability measure). Precisely, the stochastic process $S$ for the underlying price in the risk-neutral probability measure is:
...
We discretize time into $n$ uniform intervals of length $\Delta t$.

The key difference in our coverage here relative to the Black-Scholes model is that here we are developing a model for optimal exercise (and hence, pricing) of American Options whereas the Black-Scholes Partial Differential Equation is for pricing a derivative with a fixed payoff at a fixed point in time (eg: European Call and Put Options). Of course, a special case of American Options is indeed European Options. It's important to note that here we are tackling the much harder problem of the ideal timing of exercise of the American Option.
